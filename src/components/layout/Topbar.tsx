    ...
    >
      ...
      <div className="flex items-center gap-3 bg-bg border border-border px-4 py-2.5 rounded-xl w-[400px] focus-within:ring-2 focus-within:ring-primary/20 focus-within:border-primary transition-all">
        ...
        <input 
          ...
          className="bg-transparent border-none outline-none text-sm w-full font-medium text-text-1 placeholder:text-text-2"
        />
      </div>
      ...
      <div className="flex items-center gap-4">
              ...
              <div className="max-h-80 overflow-y-auto scroll-elite py-2">
                {activeAlerts.length === 0 ? (
                  <div className="py-12 text-center">
                    ...
                    <p className="text-xs font-bold text-text-2 uppercase tracking-widest">Tudo em dia!</p>
                  </div>
                  ...
                  <div key={alert.id} className="px-5 py-4 hover:bg-bg/50 transition-colors cursor-pointer border-b border-border/50 last:border-none">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-[9px] font-bold text-text-2">{format(new Date(alert.triggered_at), 'HH:mm')}</span>
                    </div>
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <p className="text-[8px] font-bold text-text-2 uppercase tracking-tighter truncate">Contrato #{alert.contract_id.substring(0, 8)}</p>
                    </div>
                  </div>
                ))}
              </div>
        ...
        <div className="flex items-center gap-4 pl-4 border-l border-border h-10">
          <div className="hidden lg:block text-right">
             ...
             <p className="text-[8px] font-bold text-text-2 uppercase tracking-widest mt-0.5">Sistema RDY Online</p>
          </div>
