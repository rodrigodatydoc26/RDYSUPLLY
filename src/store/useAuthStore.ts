  ...
  persist(
    (set, get) => ({
      ...
      refreshProfile: async () => {
        ...
        // Set user instantly from auth metadata — zero network wait, UI unblocks immediately
        set({
          user: {
            id: session.user.id,
            name: session.user.user_metadata?.name || session.user.email?.split('@')[0] || 'Usuário',
            email: session.user.email ?? '',
            role: (session.user.user_metadata?.role as Profile['role']) ?? 'technician',
            active: true,
            created_at: session.user.created_at,
          },
        });

        // Sync full profile from DB in background — updates role, name, active from DB truth
        ...
        if (!error && profile) set({ user: profile as Profile });
      },
